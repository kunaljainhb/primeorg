...
export default function VendorTenderList() {
  ...
  const filterConfig: FilterDropdown[] = [
    {
      ...
      options: [
        ...
        ...categories.map(cat => ({ label: t(cat), value: cat }))
      ],
  ...
  if (selectedCategories.length > 0) {
    activeChips.push({
      label: `${t('Categories:')} ${selectedCategories.map(cat => t(cat)).join(', ')}`,
      onRemove: () => setSelectedCategories([])
    });
  }
  if (tenderStatusFilter !== 'all') {
    activeChips.push({
      label: `${t('Status:')} ${tenderStatusFilter === 'published' ? t('Open') : t('Closed')}`,
    ...
    <div className="space-y-8 text-start">
      ...
      <Card className="shadow-card border-none">
        <CardContent className="p-6">
          <SearchFilterBar
            ...
            placeholder={t('Search by Tender ID or title...')}
      ...
      {paginatedTenders.length > 0 ? (
        <div className="space-y-8">
                ...
                <CardContent className="p-6 flex flex-col justify-between flex-1 gap-5">
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      <ArrowRight className={`h-3.5 w-3.5 ${language === 'ar' ? 'scale-x-[-1]' : ''}`} />
                    </Button>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="flex items-center justify-between border-t border-gray-100 pt-4 px-2">
            <span className="text-sm text-gray-500 font-medium">
              {t('Showing')} {filteredTenders.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1} {t('to')} {Math.min(currentPage * itemsPerPage, filteredTenders.length)} {t('of')} {filteredTenders.length} {t('Tenders')}
            </span>
            <div className="flex items-center gap-2">
              <Button 
                variant="outline" 
        ...
        <EmptyState
          ...
          description={t("We couldn't find any Tender matching your query or selected filters. Try clearing your filters or searches to view all opportunities.")}
