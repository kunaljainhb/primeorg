...
export default function VendorMessages() {
  ...
  const [threads, setThreads] = useState([
    {
      ...
      unread: 0,
      time: '10:30 AM',
      lastMsg: 'Hello, we have uploaded the consolidated specs.',
      messages: [
    ...
    {
      id: 2,
      name: t('Procurement Board'),
      subtitle: t('Tender: Office Renovation Project'),
      unread: 0,
      time: 'Yesterday',
      lastMsg: 'Thank you, we will submit the files shortly.',
      messages: [
        ...
        { sender: 'vendor', text: 'Thank you, we will submit the files shortly.', timestamp: 'Yesterday, 4:00 PM' }
      ]
    }
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const responseMsg = {
        sender: 'admin',
        text: 'Thank you for your message. The procurement team has received your update and will respond shortly.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
      ...
      <div className="flex-1 flex border border-gray-200/80 rounded-2xl overflow-hidden bg-white shadow-xs">
        ...
        <div className="w-full sm:w-[320px] md:w-[360px] border-r border-gray-150 flex flex-col shrink-0">
          <div className="p-4 bg-gray-50/50 border-b border-gray-150/70">
            <h2 className="font-bold text-gray-800 text-sm tracking-wide uppercase">{t('Chat Threads')}</h2>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
            {threads.map(thread => (
              <div
                ...
                className={`p-4 cursor-pointer hover:bg-gray-50/70 transition-colors flex items-start justify-between gap-3 ${
