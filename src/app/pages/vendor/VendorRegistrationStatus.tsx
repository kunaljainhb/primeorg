...
export default function VendorRegistrationStatus() {
  ...
  const handleResubmit = () => {
    if (!companyData.nameEn.trim() || !companyData.nameAr.trim() || !companyData.tradeLicense.trim()) {
      toast.error(t('Please fill in all mandatory legal details.'));
      return;
    }

    if (status === 'correction' && tradeLicenseStatus === 'blurry') {
      toast.error(t('Please upload a clear replacement for the blurry Trade License document.'));
      return;
    }
    ...
    setTimeout(() => {
      ...
      setIsEditing(false);
      setStatus('pending');
      toast.success(t('Your corrected registration details and documents have been resubmitted successfully!'));
    }, 1800);
  };
  ...
  const renderStatusCard = () => {
    switch (status) {
      ...
      case 'approved':
        return (
          <Alert className="border-emerald-100 bg-emerald-50/50 shadow-card rounded-xl p-5 flex gap-4 transition-all duration-300 text-start">
            ...
            <Alert className="border-rose-100 bg-rose-50/50 shadow-card rounded-xl p-5 flex gap-4 transition-all duration-300">
              ...
              <div className="space-y-1">
                <AlertTitle className="text-base font-bold text-rose-900 leading-tight">
                  {t("Registration Application Rejected")}
                </AlertTitle>
                <AlertDescription className="text-sm text-rose-800 leading-relaxed">
                  {t("Your vendor registration application was evaluated by the procurement board and was unfortunately rejected. Please inspect the official rejection reasons and comments below.")}
                </AlertDescription>
              </div>
            </Alert>
            ...
            <Card className="border-none shadow-card bg-rose-50/30">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold flex items-center gap-2 text-rose-800">
                  ...
                  {t("Official Auditor Remarks")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-rose-700 leading-relaxed font-medium">
                  {t("The uploaded compliance credentials do not meet the minimum validity guidelines (less than 6 months validity remaining). Additionally, your business category listing does not align with the verified trade license activities.")}
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-4 animate-fadeIn text-start">
            <Alert className="border-amber-100 bg-amber-50/40 shadow-card rounded-xl p-5 flex gap-4 transition-all duration-300">
              ...
              <div className="space-y-1">
                ...
                <AlertDescription className="text-sm text-amber-800 leading-relaxed">
                  {t("Your registration application requires immediate corrections before review can proceed. Please update the fields highlighted below and upload clear replacement documents as indicated.")}
                </AlertDescription>
              </div>
            </Alert>
            ...
            <Card className="border-none shadow-card bg-amber-50/30">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2 text-amber-800">
                  ...
                  {t("Audit Corrections Checklist")}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="text-xs text-amber-700 leading-relaxed font-semibold">
                  1. <strong className="text-amber-900">{t("Arabic Legal Name spelling:")}</strong> {t("The current Arabic name contains a typographical mismatch against official registration.")}
                </div>
                <div className="text-xs text-amber-700 leading-relaxed font-semibold">
                  2. <strong className="text-amber-900">{t("Trade License Document:")}</strong> {t("The uploaded license document copy is blurry and unreadable. Please upload a clear replacement.")}
                </div>
                <div className="text-xs text-amber-700 leading-relaxed font-semibold">
                  3. <strong className="text-amber-900">{t("Trade License Number:")}</strong> {t("The Trade License number entered does not match the verified company directory.")}
                </div>
                <div className="mt-4 flex justify-end">
                  <Button 
                    onClick={() => navigate('/vendor/profile-setup')}
                    className="bg-amber-500 hover:bg-amber-600 text-white font-bold transition-all shadow-sm flex items-center gap-2 rounded-button text-xs h-9 px-4 active:scale-98 cursor-pointer"
                  >
                    <RotateCcw className="h-4 w-4" />
                    {t("Resubmit Application Form")}
                  </Button>
                </div>
              </CardContent>
            </Card>
  ...
  return (
    <div className="min-h-screen p-8 bg-[#F7F9FC] relative">
      ...
      <div className="mx-auto max-w-4xl space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2 text-start">
          ...
          <div className="flex items-center gap-4 p-3.5 rounded-xl border border-gray-100 bg-white shadow-xs">
            <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{t("Admin Simulator")}</div>
        ...
        <Card className="border-none shadow-card bg-white text-start">
          <CardHeader className="px-6 pt-6 pb-2">
            <div className="flex items-center justify-between gap-4">
              <div>
                <CardTitle className="text-lg font-bold text-gray-800">{t("Legal Profile & Registration Form")}</CardTitle>
                <CardDescription className="text-xs text-gray-400 font-medium">
                  {isEditing 
                    ? t("Resubmission mode: Edit details and upload readable files below") 
                    : t("Submitted application parameters")}
                </CardDescription>
              </div>
              {status === 'correction' && !isEditing && (
                <Button 
                  size="sm" 
                  onClick={() => navigate('/vendor/profile-setup')}
                  className="bg-amber-500 hover:bg-amber-600 text-white font-bold border-none transition-all flex items-center gap-1.5 shadow-sm rounded-button text-xs h-9 px-4 active:scale-98 cursor-pointer"
                >
                  <RotateCcw className="h-4 w-4" />
                  {t("Edit & Resubmit")}
                </Button>
            ...
            <div className="space-y-6">
              <Table>
                <TableBody>
                  ...
                  <TableRow className="hover:bg-transparent border-b border-gray-100">
                    ...
                    <TableCell className="py-4">
                      {isEditing ? (
                        ...
                        <span className="font-semibold text-sm text-gray-800 block max-w-md">{companyData.nameAr}</span>
                      )}
                    </TableCell>
                  </TableRow>
                  ...
                  {/* Address */}
                  <TableRow className="hover:bg-transparent border-b border-gray-100">
                    <TableCell className="font-semibold text-sm pl-0 py-4 text-gray-500">{t("Company Address *")}</TableCell>
                    <TableCell className="py-4">
                      {isEditing ? (
                        <Input 
                  ...
                  {/* City/Country */}
                  <TableRow className="hover:bg-transparent border-b border-gray-100">
                    <TableCell className="font-semibold text-sm pl-0 py-4 text-gray-500">{t("City & Country *")}</TableCell>
                    <TableCell className="py-4">
                      {isEditing ? (
                        ...
                        <span className="font-semibold text-sm text-gray-800">{t(companyData.city)}, {t(companyData.country)}</span>
                      )}
                    </TableCell>
                  </TableRow>
                  ...
                  {/* VAT Number */}
                  <TableRow className="hover:bg-transparent border-b border-gray-100">
                    <TableCell className="font-semibold text-sm pl-0 py-4 text-gray-500">{t("VAT TRN (Tax Registration Number) *")}</TableCell>
                    <TableCell className="py-4">
                      {isEditing ? (
                        <Input 
                  ...
                  <TableRow className="hover:bg-transparent border-none">
                    <TableCell className="font-semibold text-sm pl-0 py-4 text-gray-500">{t("Service Category *")}</TableCell>
                    <TableCell className="py-4">
                      {isEditing ? (
                        <Input 
