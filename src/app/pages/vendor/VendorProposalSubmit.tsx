...
export default function VendorProposalSubmit() {
  ...
  if (!tender) {
    return <div className="p-6 text-center">{t('Tender not found')}</div>;
  }
    ...
    <div className="space-y-8 font-sans text-start">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Card className="gap-0 h-auto">
          <CardHeader className="border-b border-gray-100 pt-4 px-6 !pb-2">
            <CardTitle className="text-lg font-bold text-gray-900">{t('Technical Approach')}</CardTitle>
          </CardHeader>
          <CardContent className="!pt-4 px-6 pb-4 space-y-6">
            <div className="space-y-2">
              <Label htmlFor="technicalDescription" className="text-sm font-bold text-gray-700">{t('Approach *')}</Label>
              <RichTextEditor
            ...
            <div className="space-y-2">
              ...
              <div className="relative group cursor-pointer border-2 border-dashed border-gray-200 rounded-xl p-5 hover:bg-gray-50/50 hover:border-[var(--fnrc-primary-green)]/40 transition-all duration-200 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-sm font-semibold text-gray-700">{t('Upload your technical file')}</span>
                <span className="text-xs text-gray-400 mt-1">{t('PDF format (Maximum file size: 10MB)')}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="gap-0 h-auto">
          <CardHeader className="border-b border-gray-100 pt-4 px-6 !pb-2">
            <CardTitle className="text-lg font-bold text-gray-900">{t('Commercial Specifications')}</CardTitle>
          </CardHeader>
          <CardContent className="!pt-4 px-6 pb-4 space-y-6">
            <div>
              <Label className="text-sm font-bold text-gray-700 mb-4 block">{t('Cost Breakdown (AED) *')}</Label>
              <div className="overflow-hidden">
                <Table>
                  <TableHeader className="bg-transparent">
                    <TableRow>
                      <TableHead className="w-[45%] font-bold text-gray-800 text-xs uppercase tracking-wider py-4 text-start">{t('Description')}</TableHead>
                      <TableHead className="text-end w-[20%] font-bold text-gray-800 text-xs uppercase tracking-wider py-4">{t('Unit Price (AED)')}</TableHead>
                      <TableHead className="text-end w-[12%] font-bold text-gray-800 text-xs uppercase tracking-wider py-4">{t('Quantity')}</TableHead>
                      <TableHead className="text-end w-[18%] font-bold text-gray-800 text-xs uppercase tracking-wider py-4">{t('Amount (AED)')}</TableHead>
                  ...
                  <TableBody>
                    {costItems.map((item) => (
                      <TableRow key={item.id} className="hover:bg-gray-50/20">
                        ...
                        <TableCell className="py-3 text-end">
                          <span className="text-sm font-semibold text-gray-800 pe-2">
                    ...
                    <TableRow className="hover:bg-transparent">
                      <TableCell colSpan={5} className="py-4 px-4 bg-gray-50/20 text-start">
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
            ...
            <div className="space-y-2">
              <Label htmlFor="paymentTerms" className="text-sm font-bold text-gray-700">{t('Payment Milestones & Terms *')}</Label>
              <RichTextEditor
                id="paymentTerms"
                rows={4}
                placeholder={t("Specify project stage payments (e.g. 20% on Mobilization, 40% on Delivery...)")}
                required
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group cursor-pointer border-2 border-dashed border-gray-200 rounded-xl p-5 hover:bg-gray-50/50 hover:border-[var(--fnrc-primary-green)]/40 transition-all duration-200 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-sm font-semibold text-gray-700">{t('Upload your commercial/BOQ file')}</span>
                <span className="text-xs text-gray-400 mt-1">{t('PDF format (Maximum file size: 10MB)')}</span>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="gap-0 h-auto">
          <CardHeader className="border-b border-gray-100 pt-4 px-6 !pb-2">
            <CardTitle className="text-lg font-bold text-gray-900">{t('Supporting Records')}</CardTitle>
          </CardHeader>
          <CardContent className="!pt-4 px-6 pb-4 space-y-6">
            <div className="flex flex-col gap-6">
              <div className="space-y-2">
                <Label htmlFor="suppDoc1" className="text-sm font-bold text-gray-700">{t('Company Profile')}</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="suppDoc2" className="text-sm font-bold text-gray-700">{t('Previous Work')}</Label>
                <div className="flex items-center gap-2">
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="sm:max-w-md p-8" aria-describedby="success-dialog-description">
          <DialogHeader>
            <DialogTitle className="text-center text-2xl font-bold text-gray-900">{t('Bid Submitted Successfully')}</DialogTitle>
          </DialogHeader>
